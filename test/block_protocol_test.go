...
func TestBlockProtocolBasic() {
	...
	if err != nil {
		t.Fatalf("Failed to encode data block: %v", err)
	}
	...
	fmt.Printf("✅ 序列化成功: 总大小=%d字节 (头部=%d + 数据=%d)\n",
		len(serialized), protocol.BlockHeaderSize, len(testData))
	...
	if err != nil {
		t.Fatalf("Failed to process incoming data: %v", err)
	}
	...
	if string(receivedBlock.Data) != string(testData) {
		t.Fatalf("Data mismatch: expected %s, got %s", string(testData), string(receivedBlock.Data))
	}

	// 验证路由信息
	routeInfo := server.ExtractRouteInfo(receivedBlock)
	if routeInfo.TargetAddress != targetAddress {
		t.Fatalf("Target address mismatch: expected %s, got %s", targetAddress, routeInfo.TargetAddress)
	}

	if routeInfo.Priority != priority {
		t.Fatalf("Priority mismatch: expected %d, got %d", priority, routeInfo.Priority)
	}

	fmt.Println("✅ 所有验证通过！")
}

func TestBlockProtocolMultiple(t *testing.T) {
	fmt.Println("\n🧪 测试多个数据块处理")
	...
	for i, tc := range testCases {
		...
		serialized, err := block.Serialize()
		if err != nil {
			t.Fatalf("Failed to serialize block %d: %v", i, err)
		}
	...
	if err != nil {
		t.Fatalf("Failed to process multiple blocks: %v", err)
	}

	if len(blocks) != len(testCases) {
		t.Fatalf("Expected %d blocks, got %d", len(testCases), len(blocks))
	}
...
func TestBlockProtocolPartialData(t *testing.T) {
	fmt.Println("\n🧪 测试部分数据处理")
	...
	for i := 0; i < len(serialized); i += chunkSize {
		...
		if err != nil {
			t.Fatalf("Failed to process chunk %d: %v", i/chunkSize, err)
		}

		if i+chunkSize >= len(serialized) {
			// 最后一个块应该能解析出完整数据
			if len(blocks) != 1 {
				t.Fatalf("Expected 1 block in final chunk, got %d", len(blocks))
			}
			fmt.Printf("✅ 最终块解析成功: %s\n", string(blocks[0].Data))
		} else {
			// 中间的块应该返回空（等待更多数据）
			if len(blocks) != 0 {
				t.Fatalf("Expected 0 blocks in partial chunk, got %d", len(blocks))
			}
